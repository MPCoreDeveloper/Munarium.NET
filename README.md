...
- **The evidence hierarchy runs in the kernel, and the artifacts behind it are served.** A plan executes in trust order, a
  ...
  progress on the wire, and the per-turn hierarchy decision persisted where an operator can read it — the turn itself
  now runs end to end in the kernel (evidence, one answer, deterministic checks, bounded correction), but a session is
  not yet a wire object: no `sessions_api` equivalent, no ordinal allocated inside the insert, no progress stream, so a
  turn's decision, context, answer and violations are returned to whoever ran it rather than stored where they could be
  read back.
