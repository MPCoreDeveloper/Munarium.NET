...
- **The evidence hierarchy runs in the kernel, and the artifacts behind it are served.** A plan executes in trust order, a
  ...
  YAML path, and the original's own worked example validates clean. What is still missing: sealed artifacts as a source
  and the session and turn operations on the wire — the turn runs end to end (intent, evidence, one answer, deterministic
  checks, bounded correction, and the decision recorded on the turn rather than inside the capped audit), a session and
  its turns are recorded with the ordinal allocated by the store, and the models a turn uses resolve through the runbook's
  task levels, but nothing yet serves the create/turn/get/close operations, so a client has no way to reach any of it.
