...
- **The evidence hierarchy runs in the kernel, and the artifacts behind it are served.** A plan executes in trust order, a
  ...
  the session and turn operations on the wire, a progress stream, and the `intent` task that resolves a query through a
  model — the turn runs end to end in the kernel (evidence, one answer, deterministic checks, bounded correction) and a
  session and its turns are now recorded, with the ordinal allocated by the store and the hierarchy decision kept on the
  turn rather than inside the capped audit, but nothing yet serves the create/turn/get/close operations, so a client has
