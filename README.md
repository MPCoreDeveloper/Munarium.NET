...
- **The evidence hierarchy runs in the kernel, and the artifacts behind it are served.** A plan executes in trust order, a
  ...
  contract's own examples. What is still missing: sealed artifacts as a source, the research profiles a runbook
  declares, progress on the wire, and the per-turn hierarchy decision persisted where an operator can read it.
