...
- **The evidence hierarchy runs in the kernel, and the artifacts behind it are served.** A plan executes in trust order, a
  ...
  the models a turn uses resolve through the runbook's task levels, but nothing yet serves the create/turn/get/close
  operations, so a client has no way to reach any of it.
