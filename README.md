...
| **Identities** | The ledger's identities are ULIDs, so an identity is sortable by the moment it was created *and carries that moment inside it* (a 48-bit count of milliseconds in its first ten characters). The kernel therefore never reads a clock: `MeshSnapshot.WrittenAt` is derived from the identities the snapshot holds, which is what makes a chronology finding reproducible by anyone who holds the snapshot — a clock read would leave a later reader unable to rebuild the answer. |
