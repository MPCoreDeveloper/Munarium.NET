...
- **The evidence hierarchy runs in the kernel, and the artifacts behind it are served.** A plan executes in trust order, a
  ...
  transports, which is the pattern the rest of the session plane will follow. What is still missing: sealed artifacts as a
  source, and the session and turn operations (`create`, `turn`, `get`, `close`) with their progress stream — a turn runs
  end to end in the kernel and a session and its turns are recorded, but they are not yet reachable from a client.
